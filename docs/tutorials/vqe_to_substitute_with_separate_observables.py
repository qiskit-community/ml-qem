...
from qiskit.algorithms.minimum_eigensolvers.minimum_eigensolver import MinimumEigensolver, MinimumEigensolverResult
...
class VQE(VariationalAlgorithm, MinimumEigensolver):
    ...
    def compute_minimum_eigenvalue(
        ...
        evaluate_energy = self._get_evaluate_energy(self.ansatz, operator, separate_observables)
        ...
        def evaluate_energy(parameters: np.ndarray) -> np.ndarray | float:
            ...
            if separate_observables:
                ...
                for coeff, pauli in zip(operator.coeffs, operator.paulis):
                    pauli = SparsePauliOp.from_operator(pauli)
                    try:
                        job = self.estimator.run(batch_size * [ansatz], batch_size * [pauli], parameters)
                        estimator_result = job.result()
                    except Exception as exc:
                        raise AlgorithmError("The primitive job to evaluate the energy failed!") from exc

                    values += np.real(coeff) * np.array(estimator_result.values)
                values = list(values)
            else:
                try:
